        ...
        <form action="{{ route('classes.update', $class) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="grade_level" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent">
                    <option value="7" {{ old('grade_level', $class->grade_level) == 7 ? 'selected' : '' }}>Kelas 7</option>
                    <option value="8" {{ old('grade_level', $class->grade_level) == 8 ? 'selected' : '' }}>Kelas 8</option>
                    <option value="9" {{ old('grade_level', $class->grade_level) == 9 ? 'selected' : '' }}>Kelas 9</option>
                </select>
            </div>
